...
;while token == * -> match('*) factor() -> loop 
   (cond
      ((eq (token state) 'TIMES)
         (match state 'TIMES)
         (factor state)
         (term-aux state))
      (t nil)
...
(defun var-part (state)
   ;if(lookahead == var) { match(var); (var-dec-list(state) )}
   (cond
      ((eq (token state) 'VAR) 
         (match state 'VAR)
         (var-dec-list state))
...
(defun typ (state)
   (cond
      ((eq (token state) 'INTEGER) (get-token state))
      ((eq (token state) 'REAL) (get-token state))
      ((eq (token state) 'BOOLEAN) (get-token state))
      (t (synerr2 state))
   )
)
...
;;if not EOF -> semantic error, next token, then recursive call
   (cond ((eq (token state) 'EOF) nil)
   (t 
      (semerr3 state)
      (get-token state)
      (check-end state))
